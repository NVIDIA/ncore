...
@click.option('--start-frame',
              ...
              help='Initial lidar frame to be exported',
              default=0)
@click.option('--end-frame',
              type=click.IntRange(min=-1, max_open=True),
              help='End lidar frame to be exported',
              default=-1)
...
def dsai_export_ply(root_dir: str, output_dir: Optional[str], sensor_id: str, start_frame: int, end_frame: int,
    ...
    if end_frame == -1:
        end_frame = sensor.get_frames_count()

    indices = range(start_frame, end_frame, step_frame)
