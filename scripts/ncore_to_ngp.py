...
def extract_dynamic_tracks(
    lidar_sensor: LidarSensor,
    lidar_frame_range: range,
    ...
    extended_lidar_frame_range = range(
        max(lidar_frame_range.start - 1, 0), min(lidar_frame_range.stop + 1, lidar_sensor.get_frames_count())
    )
    ...
    for frame_idx in extended_lidar_frame_range:
        ...
        for label in labels:
            ...
            if (
                np.linalg.norm(bbox_rig[:3]) < track_min_centroid_rig_distance
            ):  # skip labels that are too close to the rig center
                continue

            if label.track_id in all_tracks:
                # Extend existing track
                all_tracks[label.track_id]["max_global_speed"] = max(
                    all_tracks[label.track_id]["max_global_speed"], label.global_speed
                )
...
def ncore_to_ngp(
    ...
    assert not (
        output_path_experiment := output_path_data / "ngp_configs" / experiment_name
    ...
    if lidar_id:
        ...
        dynamic_tracks = extract_dynamic_tracks(
            ...
            lidar_frame_range=range(lidar_frame_start_idx, lidar_frame_stop_idx),
    ...
    for camera_id in camera_ids:
        ...
        camera_model = CameraModel.from_parameters(
            camera_model_parameters, device="cpu"  # we only project small number of points
        )
        ...
        if camera_mask_image := camera_sensor.get_camera_mask_image():
            ...
            ego_car_mask = scipy.ndimage.binary_dilation(
                np.asarray(camera_mask_image) != 0, iterations=static_camera_mask_dilations
            ).astype(np.uint8)
            # Force all non-zero values to 255
            ego_car_mask = np.where(ego_car_mask == 0, 0, 255).astype(np.uint8)
        else:
            # Initialize as empty
            ego_car_mask = np.zeros(
        ...
        for camera_frame_idx in camera_sensor.get_frame_index_range(camera_frame_start_idx, camera_frame_stop_idx):
            ...
            IMAGE_FORMAT = "jpeg"
            if not (
                frame_file_path := camera_path
                / Path(padded_index_string(camera_frame_idx)).with_suffix(f".{IMAGE_FORMAT}")
            ).exists():
                ...
                assert (
                    frame_data_format == IMAGE_FORMAT
            ...
            poses_end.append(
                nvidia_to_ngp(camera_sensor.get_frame_T_sensor_world(camera_frame_idx, FrameTimepoint.END))
            )

            # extend static mask with dynamic parts
            if not (
                mask_file_path := frame_file_path.parent / ("dynamic_mask_" + frame_file_path.stem + ".png")
            ).exists():
                ...
                for dynamic_track in dynamic_tracks.values():
                    ...
                    if torch.numel(projection.valid_indices) > 0:
                        ...
                        mask_width_padding = torch.ceil((max_x - min_x) * (track_mask_dilate_ratio - 1.0) / 2).to(
                            torch.int32
                        )
                        mask_height_padding = torch.ceil((max_y - min_y) * (track_mask_dilate_ratio - 1.0) / 2).to(
                            torch.int32
                        )
    ...
    offset = -(pose_avg * scale_factor) + np.array(
        [0.5, 0.5, 0.5]
    ...
    for camera_sensor_idx, camera_id in enumerate(camera_ids):
        ...
        if camera_sensor_idx == 0 and lidar_id:
            ...
            for lidar_frame_idx in range(lidar_frame_start_idx, lidar_frame_stop_idx):
                if not (
                    dat_path := lidar_path / Path(padded_index_string(lidar_frame_idx)).with_suffix(".dat")
                ).exists():
