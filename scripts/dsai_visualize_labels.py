...
from dsai_internal.common.visualization import LabelVisualizer
from dsai_internal.data.data3 import ShardDataLoader, LidarSensor
...
def dsai_visualize_labels(shard_file_pattern, sensor_id, start_frame, end_frame, step_frame):
    ...
    for frame_index in tqdm.tqdm(sensor.get_frame_index_range(start_frame, end_frame, step_frame)):
        ...
        viz.add_pc(sensor.get_frame_data(frame_index, 'xyz_e'), sensor.get_frame_data(frame_index, 'intensity'),
                   sensor.get_frame_data(frame_index, 'dynamic_flag'), sensor.get_frame_data(frame_index, 'timestamp_us'),
                   frame_index)
