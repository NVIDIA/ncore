...
@click.option('--end-frame', type=int, help="The end frame of the sequence", default=-1)
...
def waymo(ctx, *_, **kwargs):
    ...
    assert (ctx.obj['start_frame'] < ctx.obj['end_frame'] or ctx.obj['end_frame'] == -1), "stop frame must be larger than start frame"
    ...
    for cam in ctx.obj['cameras']:
        ...
        elif ctx.obj['end_frame'] != -1 and ctx.obj['end_frame'] > len(imgs):
        ...
        elif ctx.obj['end_frame'] == -1:
            end_frame = len(imgs)
            
        if ctx.obj['semantic_seg']:
            run_semantic_segmentation(imgs[start_frame:end_frame + 1], ctx.obj['index_digits'])

        if ctx.obj['instance_seg']:
            run_instance_segmentation(imgs[start_frame:end_frame + 1])
