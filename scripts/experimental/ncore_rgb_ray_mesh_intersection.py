...
def ncore_rgb_ray_mesh_intersection(shard_file_pattern: str, mesh_path: str, camera_id: str, start_frame: int,
                                    end_frame: int, step_frame: int, static_camera_mask_dilations: int,
                                    output_dir: str):
    ...
    # all pixels
    camera_pixels_x, camera_pixels_y = np.meshgrid(np.arange(w, dtype=np.int32),
                                                   np.arange(h, dtype=np.int32))  # [0, w-1] x [0, h-1]
    ...
    for camera_frame_index in tqdm.tqdm(camera_frame_indices):
        ...
        start_timestamp_us = camera_sensor.get_frame_timestamp_us(camera_frame_index,
                                                                  frame_timepoint=FrameTimepoint.START)
        end_timestamp_us = camera_sensor.get_frame_timestamp_us(camera_frame_index, frame_timepoint=FrameTimepoint.END)
        T_sensor_world_start = camera_sensor.get_frame_T_sensor_world(camera_frame_index,
                                                                      frame_timepoint=FrameTimepoint.START)
        T_sensor_world_end = camera_sensor.get_frame_T_sensor_world(camera_frame_index,
                                                                    frame_timepoint=FrameTimepoint.END)
        world_rays_return = camera_model.pixels_to_world_rays_shutter_pose(camera_valid_pixels,
        ...
        fid, bc, t = pcu.ray_mesh_intersection(vertices, faces, world_rays[:, :3].astype(np.float32),
                                               world_rays[:, 3:].astype(np.float32))
        ...
        pcu.save_mesh_vc(str(output_path / (padded_index_string(camera_frame_index) + '.ply')),
