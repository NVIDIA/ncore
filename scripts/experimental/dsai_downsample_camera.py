...
def downsample_ftheta(intrinsic_array, downsample) -> Tuple[np.array, Tuple[int, int]]:
    ...
    assert img_width % downsample == 0 and img_height % downsample == 0, 'incompatible downsample factor for image resolution'

    img_width_downsampled = img_width // downsample
    img_height_downsampled = img_height // downsample

    ## Downsample transform principal point location in downsampled image
    cxcy_downsampled = cxcy / downsample
    ...
    assert bw_poly(1.0) == bw_poly_downsampled(1.0 / downsample), 'computed invalid polynomial map'
...
@click.option('--downsample', type=click.IntRange(min=2, max_open=True), default=2, help='Inverse downsample factor')
def dsai_downsample_camera(root_dir: str, cam_id: int, downsample: int):
    ...
    logger.info(f'Downsampling camera {cam_id} with downsample_factor = {1.0 / downsample} <{root_dir}>')
    ...
    with open(camera_meta_paths[0], 'rb') as f:
        ...
        intrinsic_array_downsampled, (img_width_downsampled,
                                      img_height_downsampled) = downsample_ftheta(meta['intrinsic'], downsample)
