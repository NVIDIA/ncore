...
@click.command()
@click.option(
    "--source-dir", type=str, help="Path to the source folder with .json trajectories to untangle", required=True
)
...
@click.option(
    ...
    type=float,
    default=0.2,
    help="Max consecutive time difference between poses to split / untangle trajectories into individual chunks [due to DeepMap's data mixing these into a single representation] (sec)",
    required=True,
)
@click.option("--max-chunk-time-sec", type=float, default=None, help="Max individual chunk time (sec)")
@click.option(
    "--skip-session", "skip_sessions", multiple=True, type=str, help="Session-id's to be skipped", default=None
)
...
def untangle_traj_roi(**kwargs) -> None:
    ...
    for json_path in tqdm.tqdm(
        [Path(path) for path in glob.glob(params.source_dir + "/*.json")], desc=f"Parsing tangled input trajectories"
    ):
        ...
        splitpoints = (
            np.where(timestamp_differences_sec > params.untangle_max_split_time_sec)[0] + 1
        )  # increment by one to use "following" index as stop index
        if len(splitpoints):
            start_pose = 0
    ...
    if params.visualize:
        ...
        for i, trajectory_chunk in enumerate(trajectory_chunks):
            ...
            ps.register_curve_network(
                "_".join(
                    [params.roi_id, trajectory_chunk.source_session_id, str(trajectory_chunk.chunk_in_session_id)]
                ),
    ...
    for trajectory_chunk in trajectory_chunks:
        ...
        egomotion_json[poses_key] = egomotion_json[poses_key][
            trajectory_chunk.source_start_pose_idx : trajectory_chunk.source_stop_pose_idx
        ]

        output_chunk_path = output_path / (
            "_".join([params.roi_id, trajectory_chunk.source_session_id, str(trajectory_chunk.chunk_in_session_id)])
            + ".json"
        )
