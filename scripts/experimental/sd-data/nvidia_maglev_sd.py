...
from ncore.impl.common.common import uniform_subdivide_range, PoseInterpolator, time_bounds
...
class NvidiaMaglevConverter(BaseNvidiaDataConverter):
    ...
    def decode_cameras(self):
        ...
        CAMERAID_TO_CROPTRANSFORM = {
            "camera_front_wide_120fov": CropTransform(
                np.array((0, 0), dtype=np.int32), np.array((3840, 1920), dtype=np.int32), 5
            ),
            "camera_cross_left_120fov": CropTransform(
                np.array((0, 0), dtype=np.int32), np.array((3840, 1920), dtype=np.int32), 5
            ),
            "camera_cross_right_120fov": CropTransform(
                np.array((0, 0), dtype=np.int32), np.array((3840, 1920), dtype=np.int32), 5
            ),
            "camera_rear_left_70fov": CropTransform(
                np.array((0, 0), dtype=np.int32), np.array((3840, 1920), dtype=np.int32), 5
            ),
            "camera_rear_right_70fov": CropTransform(
                np.array((0, 0), dtype=np.int32), np.array((3840, 1920), dtype=np.int32), 5
            ),
            "camera_rear_tele_30fov": CropTransform(
                np.array((0, 0), dtype=np.int32), np.array((3840, 1920), dtype=np.int32), 5
            ),
        }
        ...
        for camera_id, camera_rig_name in {
            ...
            global_range_start = np.argmax(
                raw_frame_timestamps_us - self.constants.SENSORTYPE_TO_ROLLINGSHUTTERDELAY_US[sensor_type]
                >= self.global_start_timestamp_us
            )
            global_range_end = (
                np.argmax(raw_frame_timestamps_us > self.global_end_timestamp_us)
            ...
            local_range, _ = uniform_subdivide_range(
                self.shard_id, self.shard_count, 0, len(global_frame_timestamps_us)
            )
            ...
            camera_model_parameters = types.FThetaCameraModelParameters(
                ...
                types.ShutterType.ROLLING_TOP_TO_BOTTOM,
                intrinsic[0:2],
            ...
            with wds.ShardWriter(
                ...
                for continous_local_frame_index, (frame_number, frame_end_timestamp_us) in tqdm.tqdm(
                    ...
                    img_croptransformed = frame_data.get_decoded_image().resize(
                        target_resolution, box=crop_box.astype(np.float32), resample=PILImage.LANCZOS
                    )  # crop and downsample ROI
                ...
                logger.info(
                    f"> processed {len(local_frame_numbers)} local"
