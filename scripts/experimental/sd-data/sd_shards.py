...
@click.option('--shard-id',
              type=click.IntRange(min=0, max_open=True),
              default=0,
              help="Shard id in [0,N-1] controlling uniform dataset subset processing")
@click.option('--shard-count',
              type=click.IntRange(min=1, max_open=True),
              default=1,
              help="Total number of shards N to performing full dataset processing")
@click.option("--wds-shard-maxsize-mib",
              type=click.IntRange(min=1, max_open=True),
              default=4096,
              help="Maximum size per webdataset shard in MiB")
@click.option("--wds-shard-maxsamples",
              type=click.IntRange(min=1, max_open=True),
              default=100000,
              help="Target number of samples per webdataset shard")
@click.option("--min-speed-km-h",
              type=click.FloatRange(min=0.0, max_open=True),
              default=10,
              help="If provided, the minimum required speed to filter out low-motion data (kilometer/hour)")
@click.pass_context
