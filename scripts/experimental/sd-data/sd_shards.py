...
@click.option("--wds-shard-maxsamples", type=click.IntRange(min=1, max_open=True), default=1000, help="Target number of samples per webdataset shard")
@click.option("--min-speed-km-h", type=click.FloatRange(min=0.0, max_open=True), default=None, help="If provided, the minimum required speed to filter out low-motion data (kilometer/hour)")
@click.pass_context
