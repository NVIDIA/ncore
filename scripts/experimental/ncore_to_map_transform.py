...
@click.option("--ngp-config", type=str, help="Path to ngp config file with scale and translation", required=False)
@click.option(
    "--map-ref-lat",
    type=float,
...
@click.option(
    "--ncore-to-ecef-only",
    default=False,
    help="If set, will save ncore to ecef only and it does not require ngp config and map ref!",
)
@click.option(
    "--output-dir",
    type=str,
    help="Output dir for saving the transform file if ncore-to-ecef-only is true. Otherwise, will save to ngp_config dir instead.",
    required=False,
)
