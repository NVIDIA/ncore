...
def ncore_to_map_transform(shard_file_pattern: str, ngp_config: str, map_ref_lat: float, map_ref_lon: float,
    ...
    ncore_ds_path = os.path.join(ngp_config_dir, 'T_ncore_ds.npz')
    np.savez(ncore_ds_path, T_ngp_ecef=T_ngp_ecef, T_ecef_enu=T_ecef_enu, T_ncore_ecef=T_ncore_ecef)
    logger.info(f'outputted {ncore_ds_path}')
