...
class Sensor:
    ...
    def __init__(self, sensor_id: str, sensor_group: str, shard_files: list[h5py.File]):
        ...
        self._shard_metas = [json.loads(shard_file[self._sensor_group][self._sensor_id]['meta.json'][()]) for shard_file in self._shard_files]

        # TODO(janickm): add consistency check on static data across all shards?

        # Construct frame offset map / sensor frame time-range
        sensor_frame_timestamps_us = [np.array(shard_meta['frame_timestamps_us'], dtype=np.uint64) for shard_meta in self._shard_metas]

        # Offset map [0, len(s0), len(s0+s1), ... , len(s0+..+sN)]
        self._shard_frame_map = np.hstack([0, np.cumsum([len(shard_timestamps_us) for shard_timestamps_us in sensor_frame_timestamps_us])]).astype(np.uint64) 
    ...
    def get_T_sensor_rig(self) -> np.ndarray:
        ...
        return np.array(self._shard_metas[0]['T_sensor_rig'], dtype=np.float32)
