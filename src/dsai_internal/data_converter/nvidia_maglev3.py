...
from src.dsai_internal.data.types import FThetaCameraModelParameters, FrameLabel3, LabelSource, PinholeCameraModelParameters, Poses, ShutterType, TrackLabel
...
class ContainerDataWriter:
    ...
    def __init__(self, output_dir_path: Path, container_name:str, camera_ids: list[str], lidar_ids: list[str], radar_ids: list[str]):
        ...
        cameras_grp = self.container_file.require_group(self.CAMERAS_BASE_DIR)
        for camera_id in self.camera_ids:
            cameras_grp.require_group(camera_id)

        lidars_grp = self.container_file.require_group(self.LIDARS_BASE_DIR)
        for lidar_id in self.lidar_ids:
            lidars_grp.require_group(lidar_id)

        radars_grp = self.container_file.require_group(self.RADARS_BASE_DIR)
    ...
    def store_meta(self, calibration_type: str, egomotion_type: str) -> None:
        ...
        self.container_file.attrs.create('version', self.VERSION)
    ...
    def store_camera_meta(
        ...
        self.container_file[self.CAMERAS_BASE_DIR][camera_id].create_dataset('meta.json', data=json.dumps(output))
        ...
        if mask_image:
            with io.BytesIO() as buffer:
                ...
                self.container_file[self.CAMERAS_BASE_DIR][camera_id].create_dataset(
                    'mask', data=np.asarray(buffer.getvalue())).attrs.create('format', FORMAT)

            # DBG - read-back data
            # img = PILImage.open(io.BytesIO(ds[()]), formats=['png'])
    ...
    def store_camera_frame(
        ...
        frame_group = self.container_file[self.CAMERAS_BASE_DIR][camera_id].require_group(continous_frame_index_string)
    ...
    def store_lidar_meta(
        ...
        self.container_file[self.LIDARS_BASE_DIR][lidar_id].create_dataset('meta.json', data=json.dumps(output))
    ...
    def store_lidar_frame(
        ...
        # Initialize frame
        continous_frame_index_string = padded_index_string(continous_frame_index)
        frame_group = self.container_file[self.LIDARS_BASE_DIR][lidar_id].require_group(continous_frame_index_string)
...
class NvidiaMaglevConverter(BaseNvidiaDataConverter):
    ...
    def convert_sequence(self, sequence_path: Path) -> None:
        ...
        self.data_writer.store_shard_meta(session_id, self.shard_id, self.shard_count, False)
        ...
        self.data_writer.store_shard_meta(session_id, self.shard_id, self.shard_count, True)
    ...
    def decode_labels(self):
        ...
        self.track_labels, self.frame_labels = LabelProcessor.parse(labels_path, self.global_start_timestamp_us, self.global_end_timestamp_us, LabelSource.AUTOLABEL, logger)
