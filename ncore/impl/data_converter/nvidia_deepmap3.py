...
class NvidiaDeepmapConverter(BaseNvidiaDataConverter):
    ...
    def decode_poses_timestamps(self, sequence_path):
        ...
        local_pose_range = np.logical_and(
            start_timestamp_us <= self.poses_timestamps, self.poses_timestamps <= end_timestamp_us
        )
    ...
    def decode_labels(self, sequence_path):
        ...
        self.track_labels, self.frame_labels, self.track_global_dynamic_flag = LabelProcessor.parse(
            ...
            {
                self.LIDAR_SENSOR_ID: load_maglev_lidar_indexer_frame_meta(
                    Path(sequence_path) / "labels" / f"{self.LIDAR_SENSOR_ID}_meta.json"
                )
            },
            {
                self.LIDAR_SENSOR_ID: sensor_to_rig(
                    self.calibration_data[self.constants.LIDARID_TO_RIGNAME[self.LIDAR_SENSOR_ID]]
                )
            },
            self.poses_timestamps,
            self.poses,
    ...
    def decode_lidar(self, sequence_path):
        ...
        for frame_path in tqdm.tqdm(self.lidar_data_paths):
            ...
            intensities = (
                np.frombuffer(data.data.intensities, dtype=np.uint8).astype(np.float32) / 255.0
            )  # [0 .. 255] -> [0.0 .. 1.0]
            ...
            valid_idx_dist = np.less_equal(
                dist, self.constants.LIDARID_TO_FILTER_MAX_DISTANCE_METERS[self.LIDAR_SENSOR_ID]
            )
    ...
    def decode_cameras(self, sequence_path):
        ...
        for camera_id, camera_rig_name in {
            ...
            frame_timestamps = np.genfromtxt(
                os.path.join(sequence_path, "camera_data/", camera_id + ".mp4.timestamps"),
                delimiter="\t",
                dtype=np.uint64,
            )
            ...
            while success:
                ...
                assert (
                    vidcap.get(cv2.CAP_PROP_POS_FRAMES) - 1 == raw_frame_index  # represents the *next* frame to be read
                ), f"invalid frame decoded / frames were skipped for {camera_id}"
                ...
                with io.BytesIO() as buffer:
                    ...
                    PILImage.fromarray(image_rgb).save(
                        buffer, format=FORMAT, optimize=True, quality=91
            ...
            assert (
                continous_frame_index == frame_timestamps.shape[0]
            ...
            eof_camera_timestamps_us = (
                frame_timestamps[:, 1] - self.constants.SENSORTYPE_TO_EXPOSURETIME_HALF_US[sensor_type]
            ).flatten()
