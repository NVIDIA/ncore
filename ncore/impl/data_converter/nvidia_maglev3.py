...
class NvidiaMaglevConverter(BaseNvidiaDataConverter):
    ...
    def decode_cameras(self):
        ...
        for camera_id, camera_rig_name in self.constants.CAMERAID_TO_RIGNAME.items():
            ...
            if self.shard_id < self.shard_count - 1:
                assert local_frame_timestamps_us[
                    -1] < self.local_end_timestamp_us  # note: local bounds are non-inclusive in this regular case
            else:
                # last-shard or single-shard case
                assert local_frame_timestamps_us[
                    -1] <= self.local_end_timestamp_us  # note: local bounds are inclusive in this end-case
    ...
    def decode_lidars(self):
        ...
        for lidar_id, lidar_rig_name in self.constants.LIDARID_TO_RIGNAME.items():
            ...
            local_frame_numbers = raw_frame_numbers[local_range_start:local_range_end]
            local_frame_timestamps_us = raw_frame_timestamps_us[local_range_start:
                                                                local_range_end]  # corresponds to end-of-frame
            ...
            for continuous_local_frame_index, (frame_number, frame_end_timestamp_us, frame_egocompensated) in \
                ...
                xyz, unique_input_idxs = np.unique(
                    mesh.vertex_data.positions, axis=0,
                    return_index=True)  # Motion-compensated end-points in end-of-spin frame
                intensity = mesh.vertex_data.custom_attributes['intensity'][unique_input_idxs].flatten(
                ) / 2.55  # Intensities are oddly represented as [0.0 .. 2.55], normalize to [0.0 .. 1.0]
                ...
                if ts_lo is not None and ts_hi is not None:
                    ...
                    T_sensor_sensorRef_unique = T_world_sensorRef @ pose_interpolator.interpolate_to_timestamps(
                        timestamp_unique) @ T_sensor_rig
                ...
                self.data_writer.store_lidar_frame(lidar_id, continuous_local_frame_index, xyz_s, xyz_e, intensity,
                                                   timestamp, dynamic_flag, None, frame_labels, T_rig_worlds,
                                                   timestamps_us)
