...
class NvidiaMaglevConverter(BaseNvidiaDataConverter):
    ...
    def decode_poses(self):
        ...
        local_T_rig_worlds = self.global_T_rig_worlds[local_range_start:local_range_stop]
        local_T_rig_world_timestamps_us = self.global_T_rig_world_timestamps_us[local_range_start:local_range_stop]
        ...
        logger.debug(
            f"shard {self.shard_id+1}/{self.shard_count} | local_range_start {local_range_start} / local_range_stop {local_range_stop} | "
        ...
        with np.printoptions(floatmode="unique", linewidth=200):  # print in highest precision
            logger.info(
                f"> processed {local_range_stop - local_range_start} / {global_range_stop - global_range_start} local / global poses, using base pose:\n{T_rig_world_base}"
            )
    ...
    def decode_cameras(self):
        ...
        for camera_id, camera_rig_name in {
            ...
            raw_frame_timestamps_us = (
                raw_frame_timestamps_us - self.constants.SENSORTYPE_TO_EXPOSURETIME_HALF_US[sensor_type]
            )
            ...
            local_frame_numbers = raw_frame_numbers[local_range_start:local_range_stop]
            local_frame_timestamps_us = raw_frame_timestamps_us[local_range_start:local_range_stop]

            logger.debug(
                f"camera {camera_rig_name} | local_range_start {local_range_start} / local_range_stop {local_range_stop} | "
                f"{self.local_start_timestamp_us} <= t < {self.local_end_timestamp_us}"
            )
            ...
            local_camera_timestamps_us = (
                np.stack(local_frame_timestamps_us)
                if local_frame_timestamps_us.size != 0
                else np.empty_like(local_frame_timestamps_us)
            ...
            self.data_writer.store_camera_meta(
                ...
                FThetaCameraModelParameters(
                    intrinsic[2:4].astype(np.uint64),
    ...
    def decode_lidars(self):
        ...
        for lidar_id, lidar_rig_name in {
            ...
            raw_frame_egocompensated = np.full_like(
                raw_frame_timestamps_us, frames_metadata.frames_egocompensated, dtype=np.bool8
            )
            del frames_metadata
            ...
            global_frame_numbers = raw_frame_numbers[global_range_start:global_range_stop]
            global_frame_timestamps_us = raw_frame_timestamps_us[global_range_start:global_range_stop]
            ...
            local_frame_egocompensated = raw_frame_egocompensated[local_range_start:local_range_stop]
            num_local_frames = len(local_frame_numbers)

            logger.debug(
                f"lidar {lidar_rig_name} | local_range_start {local_range_start} / local_range_stop {local_range_stop} | "
                f"{self.local_start_timestamp_us} <= t < {self.local_end_timestamp_us}"
            )
            ...
            for continuous_local_frame_index, (frame_number, frame_end_timestamp_us, frame_egocompensated) in tqdm.tqdm(
                enumerate(zip(local_frame_numbers, local_frame_timestamps_us, local_frame_egocompensated)),
                total=num_local_frames,
            ):

                # Interpolate egomotion at frame end timestamp for sensor reference pose at end-of-spin time
                T_world_sensorRef = np.linalg.inv(
                ...
                xyz, unique_input_idxs = np.unique(
                    mesh.vertex_data.positions, axis=0, return_index=True
                )  # Motion-compensated end-points in end-of-spin frame
                intensity = (
                    mesh.vertex_data.custom_attributes["intensity"][unique_input_idxs].flatten() / 2.55
                )  # Intensities are oddly represented as [0.0 .. 2.55], normalize to [0.0 .. 1.0]
                ...
                if all(key in mesh.vertex_data.custom_attributes for key in ("timestamp_lo", "timestamp_hi")):
                    ...
                    ts_lo = np.array(mesh.vertex_data.custom_attributes["timestamp_lo"])[unique_input_idxs].astype(
                        np.uint32
                    )
                    ts_hi = np.array(mesh.vertex_data.custom_attributes["timestamp_hi"])[unique_input_idxs].astype(
                        np.uint32
                    )
                ...
                if ts_lo is not None and ts_hi is not None:
                    ...
                    if frame_number == global_frame_numbers[0]:
                        ...
                        if np.any(past_idxs):
                            logger.info(
                                "> snapping out-of-range point timestamps of *initial* spin to start of egomotion"
                            )
                    ...
                    if not frame_egocompensated:
                        ...
                        xyz = (
                            T_sensor_sensorRef_unique[unique_timestamp_reverse_idxs, :3, :3] @ xyz[:, :, None]
                        ).squeeze(
                            -1
                        ) + xyz_s  # N x 3
                    ...
                    if not frame_egocompensated:
                        raise ValueError(
                            "egomotion-compensated input point-cloud required if no timestamps are available"
                        )
                ...
                valid_idxs &= (
                    np.linalg.norm(xyz_s - xyz_e, axis=1)
                    < self.constants.LIDARID_TO_FILTER_MAX_DISTANCE_METERS[lidar_id]
                )
                ...
                dynamic_flag, frame_labels = LabelProcessor.lidar_dynamic_flag(
                    lidar_id, xyz, frame_end_timestamp_us, self.frame_labels, self.track_global_dynamic_flag
                )
                ...
                logger.debug(
                    f"> spin {continuous_local_frame_index+1}/{num_local_frames} | load/process/dynflag/store {time_load:.2f}/{time_process:.2f}/{time_dynflag:.2f}/{time_store:.2f}sec"
                )
