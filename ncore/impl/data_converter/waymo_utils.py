...
def extrapolate_pose_based_on_velocity(T_SDC_global: np.ndarray, v_global: np.ndarray, w_global: np.ndarray,
                                       dt_sec: float) -> np.ndarray:
...
def parse_range_image_and_segmentations(frame, laser_name, ri_index=0):
  ...
  """
    ...
    for laser in frame.lasers:
        ...
        if laser.name == dataset_pb2.LaserName.TOP:
            range_image_top_pose_str_tensor = tf.io.decode_compressed(laser_ri_return.range_image_pose_compressed,
                                                                      'ZLIB')
...
def convert_range_image_to_point_cloud(frame, laser_name, range_image, segmentation, range_image_top_pose,
  ...
  """
    cartesian_range_image, timestamps_full = convert_range_image_to_cartesian(frame, laser_name, range_image,
...
def convert_range_image_to_cartesian(frame, laser_name, range_image, range_image_top_pose, start_end_timestamps):
  ...
  """
    ...
    # [H, W, 6]
    range_image_top_pose_tensor = tf.reshape(tf.convert_to_tensor(value=range_image_top_pose.data),
                                             range_image_top_pose.shape.dims)
    # [H, W, 3, 3]
    range_image_top_pose_tensor_rotation = get_rotation_matrix(range_image_top_pose_tensor[..., 0],
                                                               range_image_top_pose_tensor[..., 1],
                                                               range_image_top_pose_tensor[..., 2])
    range_image_top_pose_tensor_translation = range_image_top_pose_tensor[..., 3:]
    range_image_top_pose_tensor = get_transform(range_image_top_pose_tensor_rotation,
    ...
    for c in frame.context.laser_calibrations:
        ...
        if len(c.beam_inclinations) == 0:  # pylint: disable=g-explicit-length-test
            beam_inclinations = compute_inclination(tf.constant([c.beam_inclination_min, c.beam_inclination_max]),
...
def extract_point_cloud_from_range_image(range_image,
  ...
  """
    with tf.compat.v1.name_scope(scope, 'ExtractPointCloudFromRangeImage',
        ...
        range_image_cartesian, timestamps = compute_range_image_cartesian(range_image_polar,
                                                                          extrinsic,
                                                                          start_end_timestamps,
...
def compute_inclination(inclination_range, height, scope=None):
  ...
  """
    with tf.compat.v1.name_scope(scope, 'ComputeInclination', [inclination_range]):
        ...
        inclination = ((.5 + tf.cast(tf.range(0, height), dtype=inclination_range.dtype)) /
                       tf.cast(height, dtype=inclination_range.dtype) * tf.expand_dims(diff, axis=-1) +
  ...
  """
    ...
    with tf.compat.v1.name_scope(scope, 'ComputeRangeImagePolar', [range_image, extrinsic, inclination]):
        with tf.compat.v1.name_scope('Azimuth'):
            ...
            # [W].
            ratios = (tf.cast(tf.range(width, 0, -1), dtype=dtype) - .5) / tf.cast(width, dtype=dtype)
            # [B, W].
...
def compute_range_image_cartesian(range_image_polar,
  ...
  """
    ...
    with tf.compat.v1.name_scope(scope, 'ComputeRangeImageCartesian',
        ...
        # Approximate timestamps based on linearization of timestamp bounds relative to azimuth angles
        # [B, H, W]
        t = 1 - (azimuth + np.pi) / (2 * np.pi)  # *clockwise* spinning lidar lidar (largest azimuth are measured first)
        range_image_timestamps = start_end_timestamps[0] + tf.cast(
            t * (start_end_timestamps[1] - start_end_timestamps[0]), dtype=start_end_timestamps[0].dtype)
        ...
        if pixel_pose is not None:
            ...
            # [B, H, W, 3]
            range_image_points = tf.einsum('bhwij,bhwj->bhwi', pixel_pose_rotation,
                                           range_image_points) + pixel_pose_translation

            range_image_center = tf.einsum('bhwij,bhwj->bhwi', pixel_pose_rotation,
            ...
            if return_local_coordinates:
                ...
                # [B, H, W, 3]
                range_image_points = tf.einsum('bij,bhwj->bhwi', world_to_vehicle_rotation,
                                               range_image_points) + world_to_vehicle_translation[:, tf.newaxis,
                                                                                                  tf.newaxis, :]
                range_image_center = tf.einsum('bij,bhwj->bhwi', world_to_vehicle_rotation,
